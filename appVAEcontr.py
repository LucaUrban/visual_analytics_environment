    ...
    if widget == "Consistency checks":
        ...
        if methodology == 'Ratio analysis':
            ...
            if cat_sel_col == '-':
                ...
                st.write(np.argwhere(tab_per >= prob_cases_per))
                list_prob_cases = [[con_checks_feature, list_countries[j], list_un_cat[i], f'{round(tab_per[i, j], 2)}%', f'{DV_fin_res[i, j]}/{tab_abs[i, j]}'] if i < tab_per.shape[0]-2 or j < tab_per.shape[1]-2
