...
if demo_data_radio == 'Demo datset' or uploaded_file is not None:
    ...
    if widget == "Anomalies check":
        ...
        st.write('To download the results select the descriptive columns you want from the next field and then click the Download data button')
    ...
    if widget == "Consistency checks":
        ...
        if methodology == 'Ratio analysis':
            ...
            trend_inst = st.selectbox('Institution to vizualize', dict_trend[trend_type])
            try:
                fig_trend = go.Figure(); lim_plot = max(table[table[con_checks_id_col] == trend_inst][con_checks_feature].values)
    ...
    if widget == "Time Similarity Aanalysis":
        ...
        for en_id in table[id_col].unique():
            if en_id != el_id:
                ...
                comm_year_tab = el_val.join(en_val, on = time_col, lsuffix = '_el', rsuffix = '_en', how = 'inner')
